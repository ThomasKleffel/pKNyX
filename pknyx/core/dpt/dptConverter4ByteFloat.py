...
class DPTConverter4ByteFloat(DPTConverterBase):
    """ DPT converter class for 4-Byte-Float (F32) KNX Datapoint Type
    ...
    def _toValue(self):
        ...
        #Logger().debug("DPTConverter4ByteFloat.dataToValue(): value=%f" % value)
        return value

    def _fromValue(self, value):
        data = struct.unpack(">L", struct.pack(">f", value))[0]  # self._fromFrame(struct.pack(">f", value))
        #Logger().debug("DPTConverter4ByteFloat._fromValue(): data=%s" % hex(data))
        self._data = data
    ...
    def _toStrDPT(self):
        ...
        if self._displayUnit and self._dpt.unit is not None:
            try:
                ...
                Logger().exception("DPTConverter4ByteFloat", debug=True)
        return s
...
if __name__ == '__main__':
    ...
    class DPTConverter4ByteFloatTestCase(unittest.TestCase):
        ...
        def setUp(self):
            ...
            self.conv = DPTConverter4ByteFloat("14.xxx")
        ...
        def test_checkValue(self):
            with self.assertRaises(DPTConverterValueError):
                self.conv._checkValue(self.conv._dpt.limits[1] * 10)
