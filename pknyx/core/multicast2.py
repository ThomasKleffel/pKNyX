...
__revision__ = "$Id: multicast.py 112 2013-06-21 14:31:07Z fma $"
...
class MulticastSocket(object):
    ...
    def joinGroup(self, address):
        ...
        self._sock.setsockopt(socket.IPPROTO_IP, socket.IP_ADD_MEMBERSHIP, value)
        #iface = "127.0.0.1"
        #self._sock.setsockopt(socket.IPPROTO_IP, socket.IP_ADD_MEMBERSHIP, socket.inet_aton(address) + socket.inet_aton(iface))
    ...
    def send(self, data, address=0):
        ...
        if self._address:
            ...
            while length < len(data):
                l = self._sock.sendto(data, (self._address, self._port))
                length += l
        else:
